...
export class MatchTableComponent {
  columnDefs = [
    ...
    { headerName: 'Mac Tarihi', field: 'match_date', filter: true, sortable: true, resizable: true },
    { headerName: 'Mac Suresi', field: 'match_duration', filter: true, sortable: true, resizable: true },
    { headerName: 'Mac Sonucu', field: 'match_result', filter: true, sortable: true, resizable: true },
  ];
    ...
    this.requestService.getHeroes().subscribe(heroes => {
      this.requestService.getMatchList().subscribe(response => {
        response.forEach(match => {
          match.hero_name = heroes.find(obj => obj.id === match.hero_id).localized_name;
          ...
          match.match_duration = this.utilityService.secondToMin(match.duration);
          match.match_date = this.utilityService.pastSecondToDate(match.start_time);
        });
  ...
  onSelectRow() {
    ...
    this.dialog.open(MatchModalComponent, { width: '500px', data: { match_id: selectedRows[0].match_id } });
  }
}
